...
import { getConfirmedTemplateIds, claimConfirmation, attachConfirmationEntry } from "@/lib/db/recurring-confirmations";
...
export async function generateDueRecurringEntries(): Promise<void> {
  ...
  for (const template of due) {
    ...
    const claimed = await claimConfirmation(template.id, month);
    if (!claimed) continue; // already generated by a concurrent call
    ...
    if (template.type === "expense") {
      ...
      await attachConfirmationEntry(template.id, month, { expenseId: expense.id });
    } else {
      ...
      await attachConfirmationEntry(template.id, month, { incomeEntryId: income.id });
    }
  }
}
